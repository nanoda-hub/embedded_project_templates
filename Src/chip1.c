...
void wifiSet(void) {
    sendstr("+++");
    delay_ms(100);
    sendstr("AT+WMODE=AP"); // 设置WiFi工作模式为AP（Access Point）
    delay_ms(10000);
    sendstr("AT+WAP=temp,12345678"); // 设置WiFi AP的SSID为temp，密码为12345678
    delay_ms(100);
    sendstr("AT+SOCK=TCPS,192.168.4.1,8080"); // 设置TCP服务器监听在端口8080
    delay_ms(100);
...
void usart1Set(void) {
    //USART1引脚时钟使能
    RCC->APB2ENR |= 0x00000004; //使能GPIOB的时钟	
    //USART1引脚设定
    GPIOA->CRH = 0x444444b4; //设定PB10（USART1的发送引脚）的CNF设定为推挽式输出引脚，模式MODE设定为输出模式/速率50M；设定PB11（USART1的接收引脚）的CNF设定为浮空输入引脚，模式MODE设定为输入模式
...
void USART3_IRQHandler(void) {
    if ((USART3->SR & 0x0020) != 0) { // 读取USART3 SR寄存器的RXNE位，0：无接收数据 1：有接收数据
